import asyncio
import time
import tracemalloc
from sqlalchemy.ext.asyncio import AsyncSession, create_async_engine
from sqlalchemy.orm import sessionmaker
from sqlalchemy import text
from sqlalchemy.future import select
import sys
import os

# add backend to path
sys.path.insert(0, os.path.abspath('backend'))
import db_models
from db_models import DBProduct

async def setup_benchmark():
    engine = create_async_engine("sqlite+aiosqlite:///:memory:", echo=False)
    async with engine.begin() as conn:
        await conn.run_sync(db_models.Base.metadata.create_all)

    async_session = sessionmaker(
        engine, class_=AsyncSession, expire_on_commit=False
    )

    async with async_session() as db:
        # insert 10000 dummy products
        products = []
        for i in range(10000):
            p = DBProduct(
                id=f"prod_{i}",
                scientific_name=f"Scientific {i}",
                common_name_tr=f"Common {i}",
                slug=f"slug_{i}",
                category="plant",
                care_level="easy",
                light_need="medium",
                water_need="medium",
                size="medium",
                pet_safe=True,
                pot_size="10cm",
                tags=[],
                short_description="Test short description",
                description="Test description",
                care_tips=[],
                meta_title="Test Title",
                meta_description="Test Meta Desc"
            )
            products.append(p)
            db.add(p)
        await db.commit()

    return async_session

async def mock_ingest(title, source_type, content):
    await asyncio.sleep(0.001)

async def run_baseline(async_session):
    async with async_session() as db:
        tracemalloc.start()
        start_time = time.time()

        products = await db.execute(select(DBProduct))
        product_tasks = []
        for p in products.scalars().all():
            content = f"# Ürün: {p.common_name_tr}\n"
            content += f"Fiyat: {p.price} TL\nKategori: {p.category}\n"
            product_tasks.append(mock_ingest(f"Ürün: {p.common_name_tr}", "product", content))

        await asyncio.gather(*product_tasks)

        end_time = time.time()
        current, peak = tracemalloc.get_traced_memory()
        tracemalloc.stop()

        print(f"Baseline took: {end_time - start_time:.4f} seconds")
        print(f"Baseline memory: Current: {current / 10**6:.2f}MB, Peak: {peak / 10**6:.2f}MB")
        return end_time - start_time, peak

async def run_optimized(async_session):
    async with async_session() as db:
        tracemalloc.start()
        start_time = time.time()

        limit = 100
        offset = 0

        while True:
            products = await db.execute(select(DBProduct).limit(limit).offset(offset))
            batch = products.scalars().all()
            if not batch:
                break

            product_tasks = []
            for p in batch:
                content = f"# Ürün: {p.common_name_tr}\n"
                content += f"Fiyat: {p.price} TL\nKategori: {p.category}\n"
                product_tasks.append(mock_ingest(f"Ürün: {p.common_name_tr}", "product", content))

            await asyncio.gather(*product_tasks)
            offset += limit

        end_time = time.time()
        current, peak = tracemalloc.get_traced_memory()
        tracemalloc.stop()

        print(f"Optimized took: {end_time - start_time:.4f} seconds")
        print(f"Optimized memory: Current: {current / 10**6:.2f}MB, Peak: {peak / 10**6:.2f}MB")
        return end_time - start_time, peak

async def main():
    print("Setting up benchmark...")
    async_session = await setup_benchmark()

    print("\nRunning baseline...")
    t1, peak1 = await run_baseline(async_session)

    print("\nRunning optimized...")
    t2, peak2 = await run_optimized(async_session)

    print(f"\nTime Improvement: {t1/t2:.2f}x faster")
    print(f"Memory Improvement: {peak1/peak2:.2f}x less memory used")

if __name__ == "__main__":
    asyncio.run(main())
