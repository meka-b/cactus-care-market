...
async def _call_exa(query: str, db: AsyncSession) -> dict:
    ...
    try:
        async with httpx.AsyncClient() as client:
            r = await client.post(
                "https://api.exa.ai/search", json=payload, headers=headers, timeout=30.0
            )
            r.raise_for_status()
            data = r.json()
            return data.get("output", {}).get(
                "content", {"entities": [], "faqs": [], "gaps": []}
            )
...
async def _call_mistral(prompt: str, db: AsyncSession) -> str:
    ...
    try:
        async with httpx.AsyncClient() as client:
            r = await client.post(
                MISTRAL_URL, json=payload, headers=headers, timeout=90.0
            )
            r.raise_for_status()
            data = r.json()
            ...
            cleaned = re.sub(
                r"^```(?:html)?\s*|\s*```$", "", content.strip(), flags=re.MULTILINE
            )
            return cleaned
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def suggest_links(req: SuggestLinksRequest, db: AsyncSession = Depends(get_db)):
    ...
    products_result = await db.execute(
        select(db_models.DBProduct.common_name_tr, db_models.DBProduct.slug)
    )
    for p in products_result.all():
        products.append({"title": p[0] or p[1], "url": f"/urun/{p[1]}"})

    blogs_result = await db.execute(
        select(db_models.DBBlogPost.title, db_models.DBBlogPost.slug).where(
            db_models.DBBlogPost.status == "published"
        )
    )
...
async def knowledge_graph(db: AsyncSession = Depends(get_db)):
    ...
    products = await db.execute(
        select(db_models.DBProduct.category, db_models.DBProduct.tags)
    )
...
async def kg_jobs(db: AsyncSession = Depends(get_db)):
    ...
    stmt = (
        select(db_models.DBKnowledgeGraphJob)
        .order_by(db_models.DBKnowledgeGraphJob.updated_at.desc())
        .limit(20)
    )
...
async def manual_scan(
    ...
    return {
        "message": f"{queued} adet taranmamış ürün listeye eklendi.",
        "queued_count": queued,
    }
...
async def trigger_job(
    ...
    stmt = select(db_models.DBKnowledgeGraphJob).where(
        db_models.DBKnowledgeGraphJob.id == job_id
    )
...
async def clear_failed_jobs(db: AsyncSession = Depends(get_db)):
    stmt = select(db_models.DBKnowledgeGraphJob).where(
        db_models.DBKnowledgeGraphJob.status == "failed"
    )
    res = await db.execute(stmt)
