...
async def identify_with_plantnet(image_bytes: bytes, plantnet_key: str, filename: str = "plant.jpg") -> Dict:
    if not plantnet_key:
...
async def generate_taxonomy_with_mistral(image_bytes: bytes, plantnet: dict, mistral_key: str, product_name: str = "", taxonomy: dict = None) -> Dict:
    if not mistral_key:
...
async def analyze_plant_image(image_bytes: bytes, db, product_name: str = "") -> Dict:
    ...
    from settings_service import get_taxonomy
    taxonomy = await get_taxonomy(db)
    plant = await identify_with_plantnet(image_bytes, keys["plantnet"])
    ...
    ai = await generate_taxonomy_with_mistral(image_bytes, plant, keys["mistral"], product_name, taxonomy)
