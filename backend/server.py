...
from taxonomy_helpers import (
    get_slug_from_taxonomy,
    compute_tags_from_taxonomy,
    get_taxonomy_names,
)
import settings_service
from auth import (
...
async def admin_rag_sync(
    ...
    try:
        ...
        old_docs_result = await db.execute(
            select(DBRagDocument.id).where(
                DBRagDocument.source_type.in_(["product", "blog"])
            )
        )
        ...
        if old_doc_ids:
            await db.execute(
                delete(DBRagChunk).where(DBRagChunk.document_id.in_(old_doc_ids))
            )
            await db.execute(
                delete(DBRagDocument).where(DBRagDocument.id.in_(old_doc_ids))
            )
            ...
            async with sem:
                async with AsyncSessionLocal() as session:
                    await rag_service.ingest_document(
                        title, source_type, content, session
                    )
                    await session.commit()

        # 2. Fetch all products in batches to optimize memory
        batch_size = 100
        offset = 0
        product_count = 0
        while True:
            products_res = await db.execute(
                select(DBProduct).limit(batch_size).offset(offset)
            )
            products_batch = products_res.scalars().all()
            ...
            for p in products_batch:
                ...
                product_tasks.append(
                    ingest_with_sem(f"Ürün: {p.common_name_tr}", "product", content)
                )
                product_count += 1
        ...
        # 3. Fetch all blogs in batches to optimize memory
        offset = 0
        blog_count = 0
        while True:
            blogs_res = await db.execute(
                select(DBBlogPost)
                .where(DBBlogPost.status == "published")
                .limit(batch_size)
                .offset(offset)
            )
            blogs_batch = blogs_res.scalars().all()
            if not blogs_batch:
                break
            ...
            for b in blogs_batch:
                ...
                blog_tasks.append(
                    ingest_with_sem(f"Makale: {b.title}", "blog", content)
                )
                blog_count += 1
        ...
        return {
            "message": f"{product_count} ürün ve {blog_count} makale başarıyla RAG sistemine senkronize edildi."
        }
    except Exception as e:
        import logging
...
async def admin_rag_upload(
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, detail="Sadece UTF-8 metin (.md, .json) desteklenir."
        )
    ...
    async def process_in_background(fname, stype, cstr):
        async with AsyncSessionLocal() as session:
            try:
                await rag_service.ingest_document(
                    filename=fname, source_type=stype, content=cstr, db=session
                )
            except Exception as e:
                logging.error(f"RAG ingest error: {e}")

    background_tasks.add_task(
        process_in_background, file.filename, source_type, content_str
    )

    return {
        "status": "success",
        "message": "Dosya arka planda işleniyor. Kısa süre sonra istatistiklere yansıyacaktır.",
    }


@api.get("/admin/rag/stats")
async def admin_rag_stats(
    user=Depends(require_admin), db: AsyncSession = Depends(get_db)
):
    ...
    docs_result = await db.execute(
        select(DBRagDocument).order_by(desc(DBRagDocument.created_at)).limit(10)
    )
    ...
    return {"documents": doc_count, "chunks": chunk_count, "recent_files": recent_docs}


@api.delete("/admin/rag/documents/{doc_id}")
async def admin_rag_delete(
    doc_id: str, user=Depends(require_admin), db: AsyncSession = Depends(get_db)
):
...
async def featured_products(limit: int = 8, db: AsyncSession = Depends(get_db)):
    stmt = (
        select(DBProduct)
        .where(DBProduct.is_published == True, DBProduct.is_featured == True)
        .order_by(desc(DBProduct.created_at))
        .limit(limit)
    )
    result = await db.execute(stmt)
    items = [
    ...
    if not items:
        ...
        stmt = (
            select(DBProduct)
            .where(DBProduct.is_published == True)
            .order_by(desc(DBProduct.created_at))
            .limit(limit)
        )
        result = await db.execute(stmt)
...
async def get_product_by_slug(slug: str, db: AsyncSession = Depends(get_db)):
    stmt = select(DBProduct).where(
        DBProduct.slug == slug, DBProduct.is_published == True
    )
    result = await db.execute(stmt)
    ...
    for cat in taxonomy.get("product_categories", []):
        if cat.get("status", "active") == "active":
            out.append(
                {
                    "slug": cat.get("slug"),
                    "title": cat.get("h1", cat.get("name")),
                    "filter": {"category": cat.get("name")},
                }
            )
...
async def admin_save_categories(
    data: dict, user=Depends(require_admin), db: AsyncSession = Depends(get_db)
):
...
async def admin_categories_generate(
    ...
    try:
        data = category_ai.generate_category_metadata_sync(
            req.name, req.type, mistral_key
        )
        return data
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def admin_system_doctor_scan(
    user=Depends(require_admin), db: AsyncSession = Depends(get_db)
):
...
async def admin_system_doctor_history(
    limit: int = 30, user=Depends(require_admin), db: AsyncSession = Depends(get_db)
):
    from db_models import DBSystemScan

    res = await db.execute(
        select(DBSystemScan).order_by(DBSystemScan.created_at.desc()).limit(limit)
    )
...
async def admin_ai_sync(
    ...
    return {"suggestion": ai, "images": saved_images}


@api.post("/admin/ai/blog-seo")
async def admin_blog_seo(
    req: BlogSEORequest, user=Depends(require_admin), db: AsyncSession = Depends(get_db)
):
    """Generate SEO suggestions for a blog post."""
    try:
        out = await ai_service.generate_blog_seo(
            req.title, db, req.excerpt or "", req.target_keywords or ""
        )
        return out
    except Exception as e:
...
async def admin_create_product(
    ...
    if data.category and data.category not in cats:
        raise HTTPException(
            status_code=400, detail=f"Geçersiz kategori: {data.category}"
        )
    ...
    if new_product.scientific_name:
        ...
        background_tasks.add_task(
            trigger_kg_blueprint_research, new_product.id, kg_job.id
        )
...
async def admin_list_products(
    ...
    stmt = (
        stmt.order_by(desc(DBProduct.created_at))
        .offset((page - 1) * limit)
        .limit(limit)
    )
    result = await db.execute(stmt)
...
async def admin_get_product_by_id(
    product_id: str, user=Depends(require_admin), db: AsyncSession = Depends(get_db)
):
...
async def admin_delete_product(
    ...
    await db.execute(
        delete(DBWishlistItem).where(DBWishlistItem.product_id == product_id)
    )
...
async def admin_stats(user=Depends(require_admin), db: AsyncSession = Depends(get_db)):
    ...
    published = (
        await db.execute(
            select(func.count(DBProduct.id)).where(DBProduct.is_published == True)
        )
    ).scalar() or 0
    total_orders = (await db.execute(select(func.count(DBOrder.id)))).scalar() or 0
    paid_orders = (
        await db.execute(select(func.count(DBOrder.id)).where(DBOrder.status == "paid"))
    ).scalar() or 0
    ...
    today_orders = (
        await db.execute(
            select(func.count(DBOrder.id)).where(DBOrder.created_at >= today_dt)
        )
    ).scalar() or 0

    rev = (
        await db.execute(
            select(func.sum(DBOrder.total)).where(DBOrder.status == "paid")
        )
    ).scalar() or 0.0
    return {
...
async def admin_get_order(
    order_id: str, user=Depends(require_admin), db: AsyncSession = Depends(get_db)
):
    result = await db.execute(select(DBOrder).where(DBOrder.id == order_id))
    order = result.scalars().first()
    if not order:
...
async def create_checkout(
    ...
    if req.applied_campaigns:
        bundle_res = await campaign_service.validate_and_calc_for_cart(
            db, req.applied_campaigns
        )
    ...
    if req.coupon_code and not applied_campaign_ids:
        coupon_stmt = select(DBCoupon).where(
            DBCoupon.code == req.coupon_code.upper().strip(), DBCoupon.is_active == True
        )
    ...
    if pay.get("status") != "success" or not payment_url:
        ...
        raise HTTPException(
            status_code=400,
            detail=f"Ödeme oluşturulamadı: {pay.get('errorMessage', 'bilinmeyen hata')}",
        )
...
async def iyzico_callback_get(
    ...
    if token:
        ...
        success = (
            res.get("status") == "success" and res.get("paymentStatus") == "SUCCESS"
        )
        ...
        if success:
            try:
                ...
                await email_service.send_order_confirmation(
                    db, order.email, {**order_dict, "status": "paid"}
                )
            except Exception as e:
...
async def iyzico_callback_post(
    order_id: str, request: Request, db: AsyncSession = Depends(get_db)
):
...
async def get_order(
    order_id: str, user=Depends(optional_user), db: AsyncSession = Depends(get_db)
):
    result = await db.execute(select(DBOrder).where(DBOrder.id == order_id))
    order = result.scalars().first()
    if not order:
...
async def my_orders(user=Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    stmt = (
        select(DBOrder)
        .where(DBOrder.user_id == user["user_id"])
        .order_by(desc(DBOrder.created_at))
        .limit(100)
    )
    result = await db.execute(stmt)
...
async def wishlist_toggle(
    payload: dict, user=Depends(get_current_user), db: AsyncSession = Depends(get_db)
):
    ...
    result = await db.execute(
        select(DBWishlistItem).where(
            DBWishlistItem.user_id == user["user_id"],
    ...
    item = DBWishlistItem(user_id=user["user_id"], product_id=product_id)
    db.add(item)
    await db.commit()
...
async def get_wishlist(
    ...
    result = await db.execute(
        select(DBWishlistItem)
        .where(DBWishlistItem.user_id == user["user_id"])
        .order_by(desc(DBWishlistItem.created_at))
        .limit(500)
    )
    ...
    prod_result = await db.execute(
        select(DBProduct)
        .where(DBProduct.id.in_(product_ids), DBProduct.is_published == True)
        .limit(500)
    )
    products = [
...
async def get_wishlist_ids(
    ...
    if not user:
        return {"ids": []}
    result = await db.execute(
        select(DBWishlistItem.product_id)
        .where(DBWishlistItem.user_id == user["user_id"])
        .limit(1000)
    )
...
async def get_product_reviews(slug: str, db: AsyncSession = Depends(get_db)):
    result = await db.execute(
        select(DBReview)
        .where(DBReview.product_slug == slug, DBReview.status == "approved")
...
async def create_review(
    ...
    user_doc = (
        (await db.execute(select(DBUser).where(DBUser.id == user["user_id"])))
        .scalars()
        .first()
    )
...
async def admin_list_reviews(
    ...
    stmt = (
        stmt.order_by(desc(DBReview.created_at)).offset((page - 1) * limit).limit(limit)
    )
...
async def admin_delete_review(
    review_id: str, user=Depends(require_admin), db: AsyncSession = Depends(get_db)
):
    result = await db.execute(select(DBReview).where(DBReview.id == review_id))
    review = result.scalars().first()
    if not review:
...
def _coupon_apply(coupon: dict, subtotal: float) -> dict:
    ...
    if subtotal < (coupon.get("min_order") or 0):
        return {
            "valid": False,
            "reason": f"Minimum sepet tutarı: ₺{(coupon.get('min_order') or 0):.2f}",
        }
    if (
        coupon.get("max_uses") is not None
        and coupon.get("used_count", 0) >= coupon["max_uses"]
    ):
        return {"valid": False, "reason": "Bu kupon kullanım limitine ulaştı"}
    if coupon.get("valid_until"):
        try:
            vu = dateparser.parse(coupon["valid_until"])
            if vu < datetime.now(timezone.utc).replace(
                tzinfo=vu.tzinfo if vu.tzinfo else timezone.utc
            ):
...
async def validate_coupon(data: CouponValidate, db: AsyncSession = Depends(get_db)):
    result = await db.execute(
        select(DBCoupon).where(
            DBCoupon.code == data.code.upper(), DBCoupon.is_active == True
        )
    )
...
async def admin_list_coupons(
    ...
    result = await db.execute(
        select(DBCoupon).order_by(desc(DBCoupon.created_at)).limit(500)
    )
    items = [
        {k: v for k, v in row.__dict__.items() if not k.startswith("_")}
        for row in result.scalars().all()
...
async def admin_create_coupon(
    data: CouponCreate, user=Depends(require_admin), db: AsyncSession = Depends(get_db)
):
...
async def admin_delete_coupon(
    coupon_id: str, user=Depends(require_admin), db: AsyncSession = Depends(get_db)
):
    result = await db.execute(select(DBCoupon).where(DBCoupon.id == coupon_id))
    doc = result.scalars().first()
    if not doc:
...
async def admin_list_campaigns(
    ...
    result = await db.execute(
        select(DBCampaign).order_by(
            DBCampaign.priority.asc(), DBCampaign.created_at.desc()
        )
    )
    items = []
...
async def admin_create_campaign(
    ...
    prim_res = await db.execute(
        select(DBProduct.id).where(DBProduct.id == data.primary_product_id)
    )
    ...
    if data.related_product_ids:
        existing_res = await db.execute(
            select(DBProduct.id).where(DBProduct.id.in_(data.related_product_ids))
        )
        ...
        if missing:
            raise HTTPException(
                status_code=400, detail=f"Bağlı ürün bulunamadı: {missing}"
            )
    ...
    if data.type == "buy_x_get_y":
        if not data.free_product_id:
            raise HTTPException(
                status_code=400,
                detail="X Al Y Ücretsiz tipi için ücretsiz ürün gerekli.",
            )
        fp_res = await db.execute(
            select(DBProduct.id).where(DBProduct.id == data.free_product_id)
        )
        if not fp_res.scalar():
            raise HTTPException(status_code=400, detail="Ücretsiz ürün bulunamadı.")
    # Type-specific value validation
    if data.type == "fixed_bundle" and (
        data.bundle_price is None or data.bundle_price < 0
    ):
        raise HTTPException(status_code=400, detail="Sabit bundle fiyatı gerekli.")
    if data.type == "percentage_bundle" and (
        data.discount_pct is None or data.discount_pct <= 0 or data.discount_pct > 100
    ):
        raise HTTPException(
            status_code=400, detail="Geçerli bir yüzde indirim girin (1-100)."
        )
    if data.type == "fixed_amount_bundle" and (
        data.discount_amount is None or data.discount_amount <= 0
    ):
    ...
    if data.type == "quantity_break" and not data.quantity_tiers:
        raise HTTPException(
            status_code=400, detail="En az bir miktar kademesi tanımlayın."
        )

    camp_data = data.model_dump()
    camp_data["quantity_tiers"] = [
        t.model_dump() if hasattr(t, "model_dump") else t
        for t in (camp_data.get("quantity_tiers") or [])
    ]
...
async def admin_update_campaign(
    ...
    if "quantity_tiers" in update_data:
        update_data["quantity_tiers"] = [
            t.model_dump() if hasattr(t, "model_dump") else t
            for t in update_data["quantity_tiers"]
        ]
...
async def admin_delete_campaign(
    campaign_id: str, user=Depends(require_admin), db: AsyncSession = Depends(get_db)
):
    result = await db.execute(select(DBCampaign).where(DBCampaign.id == campaign_id))
    doc = result.scalars().first()
    if not doc:
...
async def admin_list_campaigns(user=Depends(require_admin)):
    cursor = db.campaigns.find({}, {"_id": 0}).sort(
...
async def admin_create_campaign(data: CampaignCreate, user=Depends(require_admin)):
    # Validate primary product exists
    prim = await db.products.find_one(
        {"id": data.primary_product_id}, {"_id": 0, "id": 1}
    )
    if not prim:
        raise HTTPException(status_code=400, detail="Ana ürün bulunamadı.")
    # Validate related products exist
    if data.related_product_ids:
        existing = await db.products.find(
            {"id": {"$in": data.related_product_ids}}, {"_id": 0, "id": 1}
        ).to_list(length=100)
    ...
    if data.type == "buy_x_get_y":
        ...
        fp = await db.products.find_one(
            {"id": data.free_product_id}, {"_id": 0, "id": 1}
        )
        if not fp:
            raise HTTPException(status_code=400, detail="Ücretsiz ürün bulunamadı.")
    # Type-specific value validation
    ...
    doc["quantity_tiers"] = [
        t.model_dump() if hasattr(t, "model_dump") else t
        for t in (doc.get("quantity_tiers") or [])
    ]
...
@api.patch("/admin/campaigns/{campaign_id}")
async def admin_update_campaign(
    campaign_id: str, data: CampaignUpdate, user=Depends(require_admin)
):
    update = {
        k: v for k, v in data.model_dump(exclude_unset=True).items() if v is not None
    }
    if "quantity_tiers" in update:
        update["quantity_tiers"] = [
            t.model_dump() if hasattr(t, "model_dump") else t
            for t in update["quantity_tiers"]
        ]
...
async def get_campaigns_for_product(
...
async def calculate_campaign(
    req: BundleCalcRequest, db: AsyncSession = Depends(get_db)
):
    """Frontend bundle özet hesaplaması için. Sepet kontrolü değil, sadece anlık hesap."""
    result = await db.execute(
        select(DBCampaign).where(DBCampaign.id == req.campaign_id)
    )
    ...
    if req.selected_product_ids:
        prod_res = await db.execute(
            select(DBProduct).where(DBProduct.id.in_(req.selected_product_ids))
        )
...
async def list_blog(
    ...
    if tag:
        stmt = stmt.where(
            cast(DBBlogPost.tags, String).icontains(f'"{tag}"', autoescape=True)
        )
    if search:
        stmt = stmt.where(
            or_(
    ...
    stmt = (
        stmt.order_by(desc(DBBlogPost.published_at))
        .offset((page - 1) * limit)
        .limit(limit)
    )
    result = await db.execute(stmt)
    ...
    count_stmt = select(func.count(DBBlogPost.id)).where(
...
async def get_blog(slug: str, db: AsyncSession = Depends(get_db)):
    result = await db.execute(
        select(DBBlogPost).where(
            DBBlogPost.slug == slug, DBBlogPost.status == "published"
        )
    )
    ...
    rel_stmt = (
        select(DBBlogPost)
        .where(DBBlogPost.status == "published", DBBlogPost.slug != slug)
        .order_by(desc(DBBlogPost.published_at))
        .limit(3)
    )
    ...
    if rpi:
        rp_stmt = (
            select(DBProduct)
            .where(DBProduct.id.in_(rpi), DBProduct.is_published == True)
            .limit(20)
        )
...
async def blog_tags(db: AsyncSession = Depends(get_db)):
    result = await db.execute(
        select(DBBlogPost.tags).where(DBBlogPost.status == "published")
    )
    tag_counts = {}
    for row in result.scalars().all():
        for tag in row or []:
...
async def admin_product_search(
    ...
    stmt = select(
        DBProduct.id,
        DBProduct.common_name_tr,
        DBProduct.scientific_name,
        DBProduct.slug,
        DBProduct.images,
        DBProduct.price,
    )
...
async def admin_add_product_image(
    product_id: str,
    file: UploadFile = File(...),
    generate_alt: Optional[bool] = Form(False),
    user=Depends(require_admin),
    db: AsyncSession = Depends(get_db),
):
...
async def chat_yaver(req: ChatRequest, db: AsyncSession = Depends(get_db)):
    ...
    if ctx.get("product_slug"):
        p_res = await db.execute(
            select(DBProduct).where(DBProduct.slug == ctx["product_slug"])
        )
    ...
    try:
        out = await ai_service.chat_with_yaver(
            req.message, db, history=req.history, context=ctx
        )
        return out
    except Exception:
        logger.exception("Chat failed")
        return {
            "reply": "Üzgünüm, şu an yanıt veremiyorum. Lütfen birazdan tekrar deneyin.",
            "suggestions": [],
        }
...
async def admin_update_api_keys(
    data: APIKeysUpdate, user=Depends(require_admin), db: AsyncSession = Depends(get_db)
):
    update_data = {
        k: v for k, v in data.model_dump(exclude_unset=True).items() if v is not None
    }
    ...
    await db.execute(
        update(DBSettings).where(DBSettings.key == "global").values(value=val)
    )
    await db.commit()
    settings_service.clear_cache()
...
async def admin_update_menu(
    data: MenuUpdate, user=Depends(require_admin), db: AsyncSession = Depends(get_db)
):
    update_data = {
        k: v for k, v in data.model_dump(exclude_unset=True).items() if v is not None
    }
...
async def admin_update_general(
    data: GeneralUpdate, user=Depends(require_admin), db: AsyncSession = Depends(get_db)
):
    update_data = {
        k: v for k, v in data.model_dump(exclude_unset=True).items() if v is not None
    }
...
async def get_storefront_templates(db: AsyncSession = Depends(get_db)):
    result = await db.execute(
        select(DBSettings).where(DBSettings.key == "storefront_templates")
    )
    setting = result.scalars().first()
    if not setting:
        return {}
...
async def admin_update_storefront_templates(
    data: StorefrontTemplateUpdate,
    user=Depends(require_admin),
    db: AsyncSession = Depends(get_db),
):
    result = await db.execute(
...
async def get_public_hero_settings(db: AsyncSession = Depends(get_db)):
    result = await db.execute(
        select(DBSettings).where(DBSettings.key == "hero_content")
    )
    setting = result.scalars().first()
...
async def admin_update_hero_settings(
...
async def admin_optimize_hero(
    data: HeroOptimizeRequest,
    user=Depends(require_admin),
    db: AsyncSession = Depends(get_db),
):
...
async def admin_get_blog(
    post_id: str, user=Depends(require_admin), db: AsyncSession = Depends(get_db)
):
...
async def admin_create_blog(
    ...
    if result.scalars().first():
        ...
        while True:
            res = await db.execute(
                select(DBBlogPost).where(DBBlogPost.slug == f"{base}-{i}")
            )
            if not res.scalars().first():
                break
            i += 1
...
async def sitemap(db: AsyncSession = Depends(get_db)):
    ...
    # Products
    prod_stmt = (
        select(DBProduct.slug, DBProduct.updated_at)
        .where(DBProduct.is_published == True)
        .limit(10000)
    )
    ...
    # Blog posts
    blog_stmt = (
        select(DBBlogPost.slug, DBBlogPost.updated_at)
        .where(DBBlogPost.status == "published")
        .limit(10000)
    )
    ...
    for p in products:
        body.append(
            f"<url><loc>{base}/u/{p['slug']}</loc><lastmod>{p.get('updated_at','')[:10]}</lastmod></url>"
        )
    for p in posts:
        body.append(
            f"<url><loc>{base}/blog/{p['slug']}</loc><lastmod>{p.get('updated_at','')[:10]}</lastmod></url>"
        )
    body.append("</urlset>")
        ...
        while True:
            ...
            try:
                ...
                async with AsyncSessionLocal() as session:
                    ...
                    await session.commit()
                    logger.info(
                        f"Daily system scan completed with score {scan_results['score']}"
                    )
            except Exception as e:
